...
@section('title', 'Details for ' . $order->tracking_id)
...
@section('content')
    <div class="space-y-5">
        ...
        <div class="flex items-center gap-4 flex-wrap">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2.5 flex-wrap">
                    ...
                    @php
                        $statusCfg = [
                            'assigned'   => ['label'=>'Pending Assignment', 'cls'=>'bg-amber-100 text-amber-700 border-amber-200',  'dot'=>'bg-amber-500',  'pulse'=>true],
                            'in_transit' => ['label'=>'In Transit',         'cls'=>'bg-blue-100 text-blue-700 border-blue-200',      'dot'=>'bg-blue-500',   'pulse'=>true],
                            'delivered'  => ['label'=>'Delivered',          'cls'=>'bg-emerald-100 text-emerald-700 border-emerald-200','dot'=>'bg-emerald-500','pulse'=>false],
                        ];
                ...
                <p class="text-gray-400 text-xs mt-0.5 flex items-center gap-1">
                    ...
                    Created at {{ $order->created_at->format('H:i — M d, Y') }}
                </p>
            </div>
                ...
                @elseif($order->status == 'in_transit')
                    <form action="{{ route('agent.orders.complete', $order->id) }}" method="POST">
                        ...
                        <button class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl transition-all active:scale-95 shadow-lg shadow-emerald-200">
                            <i data-lucide="package-check" class="w-4 h-4"></i> Mark as Delivered
                        </button>
                    </form>
                @endif
            </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
            ...
            {{-- ── LEFT COLUMN: Sender + Receiver (2/3 width) ── --}}
            <div class="xl:col-span-2 space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
                        ...
                        <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Sender</span>
                    </div>
                ...
                <div class="flex items-center gap-3 px-2">
                    ...
                    <div class="flex items-center gap-1.5 px-3 py-1.5 bg-primary-600 text-white rounded-full text-[0.6rem] font-bold uppercase tracking-wider shadow-md shadow-primary-200 flex-shrink-0">
                        <i data-lucide="arrow-down" class="w-3 h-3"></i> Destination
                    </div>
                ...
                <div class="bg-white rounded-2xl border border-primary-200 shadow-sm overflow-hidden ring-1 ring-primary-100">
                    <div class="px-5 py-3 bg-gradient-to-r from-primary-50 to-white border-b border-primary-100 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs font-bold text-primary-600 uppercase tracking-wider">Receiver</span>
                        </div>
                        <span class="text-[0.6rem] font-bold text-primary-400 bg-primary-50 border border-primary-100 px-2 py-0.5 rounded-full uppercase tracking-wider">Delivery Point</span>
                    </div>
                    <div class="p-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <div>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
                        ...
                        <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Shipment Specs</span>
                    </div>
                    <div class="divide-y divide-gray-50">
                        <div class="flex items-center justify-between px-5 py-3 text-sm">
                        <span class="text-gray-400 font-medium flex items-center gap-1.5">
                            <i data-lucide="scan-barcode" class="w-3.5 h-3.5"></i> Tracking ID
                        </span>
                        ...
                        <div class="flex items-center justify-between px-5 py-3 text-sm">
                        <span class="text-gray-400 font-medium flex items-center gap-1.5">
                            <i data-lucide="scale" class="w-3.5 h-3.5"></i> Weight
                        </span>
                        ...
                        <span class="text-gray-400 font-medium flex items-center gap-1.5">
                            ...
                            <span class="font-semibold text-gray-700 text-xs">{{ $order->created_at->format('M d, Y') }}</span>
                        </div>
                        <div class="flex items-center justify-between px-5 py-3 text-sm">
                        <span class="text-gray-400 font-medium flex items-center gap-1.5">
                        ...
                        <div class="flex items-center justify-between px-5 py-3 text-sm">
                        <span class="text-gray-400 font-medium flex items-center gap-1.5">
                            <i data-lucide="user-circle" class="w-3.5 h-3.5"></i> Customer
                        </span>
                            @if($order->customer)
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
                        ...
                        <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">Shipment Timeline</span>
                    </div>
                    <div class="p-5">
                        @php
                            $stages = [
                                ['key'=>'assigned',   'label'=>'Assigned',      'sub'=>'Order assigned to agency',      'icon'=>'clipboard-check',
                                 'done'=> in_array($order->status,['assigned','in_transit','delivered'])],
                                ['key'=>'in_transit', 'label'=>'In Transit',    'sub'=>'Shipper picked up & en route',  'icon'=>'truck',
                                 'done'=> in_array($order->status,['in_transit','delivered'])],
                                ['key'=>'delivered',  'label'=>'Delivered',     'sub'=>'Package received by receiver', 'icon'=>'package-check',

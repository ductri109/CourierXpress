...
@section('content')
    ...
    {{-- View-level pagination handling (Keeps Controller intact) --}}
    @php
    ...
    <div class="flex flex-col h-full space-y-5">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-950">Customer Management</h2>
                <p class="text-gray-400 text-sm mt-0.5">Search, filter addresses, and manage sender/recipient details.</p>
            </div>
        </div>
            ...
            <form method="GET" action="{{ route('agent.customers.index') }}" id="filter-form">
                <div class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex-1 min-w-[180px] relative">
                        <label class="text-[0.65rem] font-bold text-gray-400 uppercase tracking-wider block mb-1.5">Customer</label>
                        <div class="relative">
                    ...
                    {{-- Address Filter --}}
                    <div class="flex-1 min-w-[180px] relative">
                        <label class="text-[0.65rem] font-bold text-gray-400 uppercase tracking-wider block mb-1.5">Address</label>
                        <div class="relative">
                            ...
                            <input type="text" name="address" id="address-input" value="{{ request('address') }}"
                                   placeholder="District, ward, street..."
                                   autocomplete="off"
                                   class="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 transition-all">
                        </div>
                    </div>
                ...
                <div class="flex items-center gap-2 mt-3 flex-wrap">
                    <span class="text-[0.65rem] font-bold text-gray-400 uppercase tracking-wider">Joined Date:</span>
                    <button type="button" onclick="setDateRange('today')" class="text-xs px-2.5 py-1 rounded-lg border border-gray-200 text-gray-500 hover:border-primary-400 hover:text-primary-600 transition-all font-medium">Today</button>
                    <button type="button" onclick="setDateRange('yesterday')" class="text-xs px-2.5 py-1 rounded-lg border border-gray-200 text-gray-500 hover:border-primary-400 hover:text-primary-600 transition-all font-medium">Yesterday</button>
                    <button type="button" onclick="setDateRange('week')" class="text-xs px-2.5 py-1 rounded-lg border border-gray-200 text-gray-500 hover:border-primary-400 hover:text-primary-600 transition-all font-medium">Past 7 Days</button>
                    <button type="button" onclick="setDateRange('month')" class="text-xs px-2.5 py-1 rounded-lg border border-gray-200 text-gray-500 hover:border-primary-400 hover:text-primary-600 transition-all font-medium">This Month</button>
                </div>
            </form>
        </div>
        ...
        <div id="no-result-banner" class="hidden items-center gap-3 bg-amber-50 border border-amber-200 text-amber-800 px-5 py-4 rounded-2xl">
            ...
            <p class="text-sm font-semibold">No customers match the current filters.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex-1">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <h3 class="font-bold text-gray-900">Customer Directory</h3>
                    @if(request()->hasAny(['search','address','date_from','date_to']))
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs font-bold bg-gray-100 text-gray-600 px-3 py-1 rounded-full" id="customer-count">{{ method_exists($customers, 'total') ? $customers->total() : $customers->count() }} clients</span>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tr class="bg-gray-50/80 border-b border-gray-100 text-[0.65rem] font-bold text-gray-400 uppercase tracking-wider">
                        ...
                        <th class="px-5 py-3.5 text-center">Shipped Orders</th>
                    </tr>
                    </thead>
                    ...
                    @forelse($customers as $user)
                        ...
                        <tr class="customer-row hover:bg-gray-50/60 transition-colors"
                            ...
                            <td class="px-5 py-4">
                                <span class="flex items-center gap-1.5 text-gray-880 font-semibold text-xs mb-1">
                    ...
                    @empty
                        <tr id="empty-row">
                            <td colspan="5" class="py-14 text-center">
                                ...
                                <p class="text-sm font-medium text-gray-400">No customers found in the system.</p>
                            </td>
                        </tr>
                    @endforelse
                    </tbody>
    ...
    <script>
        ...
        function clientFilter() {
            ...
            if (cntEl) {
                if (q !== '' || addr !== '') {
                    cntEl.textContent = visible + ' matching (this page)';
                } else {
                    cntEl.textContent = '{{ method_exists($customers, "total") ? $customers->total() : $customers->count() }} clients';
                }
            }
